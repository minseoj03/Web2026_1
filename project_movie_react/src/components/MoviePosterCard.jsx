/**
 * MoviePosterCard - 내 페이지용 영화 포스터 카드
 * hover 시 추천 버튼, 메뉴 버튼, 한줄평 표시
 * watched / wishlist 두 타입 지원
 */
  ...
  return (
    ...
    >
      <div className={`aspect-[2/3] rounded-xl bg-gradient-to-br ${gradient || 'from-gray-400 to-gray-600'} grid place-items-center text-white font-extrabold text-sm p-3 text-center shadow-sm mb-2 relative overflow-hidden`}>
        ...
        {/* Recommend button */}
        <button
          onClick={(e) => { e.stopPropagation(); onRecommend?.(movie) }}
        ...
        {/* Menu button */}
        <button
          onClick={(e) => { e.stopPropagation(); onMenu?.(movie) }}
      ...
      <p className="text-[10px] text-gray-400">{type === 'watched' ? `${date} 시청` : `${addedDate} 담음`}</p>
  ...
  return (
    <>
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="shrink-0 w-[calc((100%-64px)/5)] min-w-[clamp(100px,10vw,130px)] max-lg:w-[calc((100%-32px)/3)] max-md:w-[calc((100%-16px)/2.5)] max-md:min-w-[clamp(90px,9vw,110px)] animate-pulse">
