      ...
      {!error && (
        <div className="relative">
          {!loading && filteredMovies.length > 0 && (
            <button onClick={() => scroll(-1)} className="absolute left-[-12px] top-1/2 -translate-y-1/2 w-9 h-9 bg-white border border-gray-200 rounded-full grid place-items-center shadow-sm z-10 hover:border-[#7c5cff] hover:text-[#7c5cff] transition">‹</button>
          )}
          ...
          <div ref={scrollRef} className="flex gap-4 overflow-x-auto scrollbar-hide pb-2">
            {loading ? (
              ...
              filteredMovies.map(m => (
                <div key={m.id} className="relative">
                  <MoviePosterCard
                    ...
                    onMenu={() => setMenuMovie(menuMovie?.id === m.id ? null : m)}
                  />
                  ...
                  {menuMovie?.id === m.id && (
                    <div className="absolute top-10 right-2 bg-white border border-gray-200 rounded-lg shadow-lg p-1 z-20 min-w-[100px] animate-[fadeIn_0.15s_ease]">
                      {type === 'watched' && (
                        <button
                          onClick={() => { onEdit?.(m); setMenuMovie(null) }}
                      ...
                      <button
                        onClick={() => { onDelete?.(m); setMenuMovie(null) }}
              ...
              <EmptyState
                icon={searchQuery ? '🔍' : type === 'watched' ? '🎬' : '❤️'}
                title={searchQuery ? '검색 결과가 없어요' : type === 'watched' ? '아직 본 영화가 없어요' : '찜한 영화를 추가해보세요'}
                description={searchQuery ? '다른 키워드로 검색해보세요.' : type === 'watched' ? '영화를 추가하고 별점과 한줄평을 남겨보세요.' : '마음에 드는 영화를 찜해두면 나중에 쉽게 찾을 수 있어요.'}
          ...
          {!loading && filteredMovies.length > 0 && (
            <button onClick={() => scroll(1)} className="absolute right-[-12px] top-1/2 -translate-y-1/2 w-9 h-9 bg-white border border-gray-200 rounded-full grid place-items-center shadow-sm z-10 hover:border-[#7c5cff] hover:text-[#7c5cff] transition">›</button>
          )}
        </div>
      )}
    </div>
